...
class NotificationsController extends NotificationsAppController {
    ...
    public function read($id = null) {
        ...
        if (!isset($item['Notification']['target'])) {
            $target = $this->redirect($this->referer('/notifications/getList1'.(AuthComponent::user('id'))));
        } else {
	...
	public function getlist1($userId = null, $limit = 10) {
        $this->layout = 'ajax';
        if ($userId && $userId == AuthComponent::user('id')) {
            $this->set('result', $this->Notification->getList1($userId, $limit));
        } else {
            $this->set('result', null);
        }
    }
